...
function IconComment({ size = 16, active = false }: { size?: number; active?: boolean }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none"
      xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style={{ flexShrink: 0 }}>
      <path
...
export default function ReflexaoView({ reflexao, autorSlug }: Props) {
  ...
  const [commentCount, setCommentCount] = useState<number>(
    reflexao.commentCount ?? 0
  );
  // Banner de login inline (para quando não está logado e clica em Comentar)
    ...
    <div style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        ...
        <h1 style={{
          ...
          color: "var(--text-1)", lineHeight: 1.2, margin: 0,
        }}>
          {reflexao.titulo}
        </h1>
        <p style={{ fontSize: "0.85rem", color: "var(--text-3)", margin: 0 }}>
          Por{" "}
          <Link href={`/perfil/${autorSlug}`}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        {paragrafos.map((p, i) => (
          <p key={i} style={{
            fontSize: "1rem", color: "var(--text-2)", lineHeight: 1.75, margin: 0,
          }}>
            {p}
          </p>
        ))}
      ...
      <div style={{
        ...
        {/* Botão Comentar — faz scroll até a seção de comentários */}
        <button
      ...
      {/* Banner de login inline — aparece ao clicar Amei ou Comentar sem estar logado */}
      {showLoginBanner && (
